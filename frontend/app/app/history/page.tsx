...
export default function HistoryPage() {
  ...
  return (
    <>
      <div className="mb-8">
        <h1 className="font-display text-[24px] font-bold tracking-tight text-[#0B1739]">Generation History</h1>
      ...
      {!loading && history.length > 0 && (
        <div className="mb-5 flex flex-wrap items-center gap-3">
          <input
            ...
            className="flex-1 min-w-[200px] rounded-xl border border-[#E2E8F0] bg-white px-4 py-2.5 text-[13px] text-[#0B1739] outline-none transition focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/15 placeholder:text-[#94A3B8]"
          />
          <select
            value={voiceFilter}
            onChange={(e) => setVoiceFilter(e.target.value)}
            className="rounded-xl border border-[#E2E8F0] bg-white px-4 py-2.5 text-[13px] text-[#0B1739] outline-none transition focus:border-[#2563EB]"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-[#E2E8F0] bg-white px-4 py-2.5 text-[12px] font-semibold text-[#0B1739] transition hover:border-[#2563EB]"
          >
      ...
      {loading ? (
        <div className="mt-12 flex justify-center"><SpectrumBars size="lg" animate className="text-[#2563EB]" /></div>
      ) : filtered.length === 0 ? (
        <div className="mt-12 flex flex-col items-center gap-4 rounded-2xl bg-white px-6 py-14 text-center shadow-sm">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#EFF6FF] text-[#93C5FD]">
            <SpectrumBars size="lg" />
          </div>
          <div>
            <p className="text-[14px] font-semibold text-[#0B1739]">{history.length === 0 ? "No generations yet" : "No results found"}</p>
          ...
          {history.length === 0 && <Link href="/app" className="mt-2 rounded-xl bg-[#2563EB] px-5 py-2.5 text-[13px] font-semibold text-white shadow-[0_8px_24px_-6px_rgba(37,99,235,0.5)] transition hover:bg-[#1D4ED8]">Create your first voice</Link>}
        </div>
      ) : (
              ...
              return (
                <div key={item.filename} className="flex items-center justify-between rounded-2xl border border-red-200 bg-red-50 px-5 py-4">
                  ...
                  <div className="flex shrink-0 gap-2">
                    ...
                    <button type="button" onClick={() => setConfirmingDelete(null)} className="rounded-xl border border-[#E2E8F0] bg-white px-4 py-2 text-[12px] font-semibold text-[#0B1739]">Cancel</button>
                  </div>
                </div>
              );
            }
            return (
              <div key={item.filename} className="flex items-center gap-4 rounded-2xl bg-white px-5 py-4 shadow-sm transition hover:shadow-md">
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  <a href={dl} download={item.filename} className="flex h-8 w-8 items-center justify-center rounded-full bg-[#EFF6FF] text-[#2563EB] transition hover:bg-[#DBEAFE]" title="Download"><svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4" /><polyline points="7 10 12 15 17 10" /><line x1="12" y1="15" x2="12" y2="3" /></svg></a>
                  <Link href={`/app/history/${encodeURIComponent(item.filename)}`} className="flex h-8 w-8 items-center justify-center rounded-full bg-[#EFF6FF] text-[#64748B] transition hover:bg-[#DBEAFE] hover:text-[#0B1739]" title="Details"><svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6" /></svg></Link>
                  <button type="button" onClick={() => setConfirmingDelete(item.filename)} className="flex h-8 w-8 items-center justify-center rounded-full bg-[#EFF6FF] text-[#64748B] transition hover:bg-red-50 hover:text-[#FF6B6B]" title="Delete"><svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6" /><path d="M19 6l-2 14H7L5 6" /><path d="M10 11v6" /><path d="M14 11v6" /></svg></button>
                </div>
              </div>
            );
          })}
