...
export default function GenerationDetailPage() {
  ...
  if (loading) return <div className="mt-12 flex justify-center"><SpectrumBars size="lg" animate className="text-[#7C5CFC]" /></div>;
  if (!gen) return (
    <div className="mt-12 text-center">
      <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#F5F3FF] text-[#C4B5FD]"><SpectrumBars size="md" /></div>
      <p className="text-[14px] font-semibold text-[#110D2A]">Generation not found</p>
      <Link href="/app/history" className="mt-3 inline-block text-[13px] font-semibold text-[#7C5CFC]">Back to history</Link>
    </div>
  );
  ...
  return (
    <>
      <div className="mb-6">
        <Link href="/app/history" className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-[#64748B] transition hover:text-[#7C5CFC]">
        ...
        <h1 className="mt-3 font-display text-[24px] font-bold tracking-tight text-[#110D2A]">Generation Details</h1>
      </div>

      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        <div className="space-y-6">
          <section className="rounded-2xl bg-white p-6 shadow-sm">
            <h2 className="mb-4 text-[15px] font-semibold text-[#110D2A]">Audio</h2>
            <AudioPlayer src={audioUrl} autoPlay={playSignal > 0} playKey={playSignal} />
            <div className="mt-4 flex gap-2">
              <a href={audioUrl} download={gen.filename} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#110D2A] py-2.5 text-[13px] font-semibold text-white transition hover:bg-[#1E1933]">
          ...
          <section className="rounded-2xl bg-white p-6 shadow-sm">
            <h2 className="mb-3 text-[15px] font-semibold text-[#110D2A]">Original text</h2>
            <p className="text-[13px] leading-relaxed text-[#110D2A] whitespace-pre-wrap">{gen.text || "No text recorded."}</p>
          </section>
        </div>

        <div className="space-y-4">
          <section className="rounded-2xl bg-white p-6 shadow-sm">
            <h3 className="mb-4 text-[13px] font-semibold text-[#110D2A]">Metadata</h3>
            <dl className="space-y-3">
              {[
                ...
                <div key={label} className="flex items-center justify-between">
                  ...
                  <dd className="text-[12px] font-medium text-[#110D2A]">{value}</dd>
                </div>
              ))}
            </dl>
          </section>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowDeleteModal(false)}>
          <div className="mx-4 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-[16px] font-bold text-[#110D2A]">Delete this generation?</h3>
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button type="button" onClick={() => setShowDeleteModal(false)} className="rounded-xl border border-[#E2E8F0] bg-white px-4 py-2.5 text-[13px] font-semibold text-[#110D2A] transition hover:border-[#7C5CFC]">Cancel</button>
